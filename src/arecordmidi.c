...
static void record_event(const snd_seq_event_t *ev)
{
    ...
    switch (ev->type) {
        case SND_SEQ_EVENT_NOTEON:
            ...
            addKeyDown(ev->data.note.note & 0x7f);
            break;
        case SND_SEQ_EVENT_NOTEOFF:
            ...
            add_byte(track, ev->data.note.note & 0x7f);
            add_byte(track, ev->data.note.velocity & 0x7f);
            addKeyUp(ev->data.note.note & 0x7f);
            break;
        ...
        case SND_SEQ_EVENT_CONTROLLER:
            ...
            addController(ev->data.control.param & 0x7f, ev->data.control.value & 0x7f);
            break;
        ...
        case SND_SEQ_EVENT_PORT_UNSUBSCRIBED:
            printf("exiting due to port diconnection\n");
            stop=1;
            break;
#if 0	/* ignore */
...
{
    fputs("arecsplitmidi version 1.0\n", stderr);
}
...
int main(int argc, char *argv[])
{
    ...
    int c, err;
    bool keysDown = false;
    struct timespec begin, end, current;
    ...
    if (hasStarted)
    {
        // only record if we at least got one key down event
